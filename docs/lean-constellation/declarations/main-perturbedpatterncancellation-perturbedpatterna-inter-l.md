...
# `perturbedPatternA_inter_L`

Intersecting an a-anchored perturbed pattern with L recovers its L-subset.

- Kind: `theorem`
- Node: `Main.PerturbedPatternCancellation`
- Module: `UniformMissingTraceFamily.Main.PerturbedPatternCancellation.Theorems.perturbedPatternA_inter_L`
- State: `proved`
...
For every `B : PerturbedStarBlocks n r` and every finite set `Y` with `Y ⊆ B.L`, intersecting the exact deleted-pattern constructor with `B.L` recovers `Y`:
